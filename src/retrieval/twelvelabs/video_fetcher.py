...
class TwelveLabsVideoFetcher:
    """基于 TwelveLabs Retrieve API 下载视频片段
    ...
    def fetch_clip(
        ...
        if self._live_enabled:
            ...
            try:
                ...
                if stream_url:
                    if self._cut_from_stream(stream_url, start_ms, end_ms, target, video_id):
                        return target
            finally:
                lock.release()
        ...
        if local_source.exists():
            if cut_clip(local_source, start_ms, end_ms, target):
                return target

        logger.warning(
            "twelvelabs.clip_unavailable",
            video_id=video_id,
            start_ms=start_ms,
            end_ms=end_ms,
        )
    ...
    def _cut_from_stream(
        ...
        # 边界检查
        source_duration_ms = get_video_duration_ms(source_url)
        if source_duration_ms:
            if start_ms >= source_duration_ms:
                ...
                return cut_clip_with_loop(source_url, duration, target)
        ...
        # 使用通用裁剪函数
        return cut_clip(source_url, start_ms, end_ms, target)
    ...
    def _retrieve_video_payload(self, video_id: str) -> dict[str, Any] | None:
        """调用 TwelveLabs Retrieve API 获取视频元数据"""
